...
/**
 * Modal for creating a new test suite
 */
const CreateSuiteModal = ({ isOpen, onClose, onSuiteCreated }) => {
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [error, setError] = useState(null)
  const { createSuite } = useSuites()
  ...
  const onSubmit = async (data) => {
    try {
      ...
      console.log('Creating suite with data:', data)
      const newSuite = await createSuite(data)
      console.log('Created suite:', newSuite)
      
      // Notify parent component
      if (onSuiteCreated) {
        console.log('Calling onSuiteCreated callback...')
        await onSuiteCreated()
        console.log('onSuiteCreated callback completed')
      }
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-foreground">Create Test Suite</h2>
                  <p className="text-sm text-muted-foreground">Organize your test cases</p>
                </div>
              </div>
              <button
                onClick={handleClose}
